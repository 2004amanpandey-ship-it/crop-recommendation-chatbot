...
def detect_language(text):
    english_words = ["is","are","am","how","what","when","why","does","do","can","should","need"]
...
# chatbot code ke pehleadd r rhi
...
OPENROUTER_API_KEY = os.getenv("OPENROUTER_KEY") or getattr(settings, "OPENROUTER_API_KEY", None)

if not OPENROUTER_API_KEY:
    print("⚠️ Warning: OpenRouter API key not found. Chatbot will work in limited mode.")
...
def chatbot_response(message: str) -> str:
    """
    Return chatbot reply based on user message.
    Rule-based first, fallback to OpenRouter AI if API key is available.
    """
    msg = (message or "").strip().lower()

    # --- Rule-based responses ---
    if any(greet in msg for greet in ["hello", "hi", "namaste"]):
        return "नमस्ते! 😊 खेती से जुड़ा सवाल पूछिए।"
    

    # --- Fallback to OpenRouter AI ---
    if not client:
        return "Chatbot temporarily unavailable. API key missing 😅"

    try:
        # response = client.chat.completions.create(
        #     model="openai/gpt-4o-mini",
        #     messages=[
        #         {"role": "system", "content": "You are an agriculture expert. Answer in Hindi."},
...
Rules:
1. Reply in the SAME language as the user's question (Hindi or English).
2. By default give SHORT answers (2-3 lines).
3. If the user asks for "long answer", "detail", "explain", or "details", then give a detailed answer (6-8 lines).
...
"""
        ...
        return "Server busy 😅 कृपया बाद में प्रयास करें।"

# ------------------- Chat API endpoint -------------------
@csrf_exempt  # optional, remove if using proper CSRF token in AJAX
def chat_api(request):
    if request.method != "POST":
        return JsonResponse({"response": "Only POST method allowed"}, status=405)
...
def user_notices(request):
    ...
    notices = Notice.objects.filter(date__gte=last_year)
